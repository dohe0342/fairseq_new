...
class Data2VecAudioModel(BaseFairseqModel):
    ...
    def forward(
        ...
        if prefix is not None:
            prefix_padding_mask = torch.zeros(features.size()[0], prefix[0].size()[0]).type(torch.BoolTensor).to(features.device)
    ...
    def extract_features(
        self, source, padding_mask, mask=False, layer=None, prompt=None, prefix=None,
    ):
        res = self.forward(
            source,
            padding_mask,
