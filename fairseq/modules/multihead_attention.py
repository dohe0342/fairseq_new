    ...
    def forward(
        ...
        if self.self_attention:
            ...
            if prefix is not None:
                prefix[0] = prefix[0].transpose(0, 1)
                prefix[1] = prefix[1].transpose(0, 1)
                k = torch.cat([prefix[0], k], dim=0)
                v = torch.cat([prefix[1], v], dim=0)
        elif self.encoder_decoder_attention:
        ...
        if key_padding_mask is not None:
            ...
            if prefix is None: assert key_padding_mask.size(1) == src_len 
            else: key_padding_mask.size(1) == src_len+prefix[0].size(1)
